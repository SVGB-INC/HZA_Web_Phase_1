...
{
    public partial class multi_comp_search : System.Web.UI.Page
    {
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            var task = multiSearch(pts.Token, cancelSource.Token);

        }

        private async Task multiSearch(PauseToken pause, CancellationToken cancellationToken)
        {



            //labelInfoList.Text = "Fetching Company Data";
            //labelInfoList.Update();
            ...
            string tbName = Session["username"] + "_companies";
            string query = "";
            ...
            try
            {
                ...
                int txtCount = (editList.Text.Split(new[] { ',', ' ' }, StringSplitOptions.RemoveEmptyEntries).Count());
                ...
                if (txtCount <= 0)
                {
                    ...
                    using (XLWorkbook workBook = new XLWorkbook(FileUpload1.PostedFile.InputStream))
                    {
                        ...
                        //companyList = dt.Rows[0].ItemArray.Select(x => x.ToString()).ToArray();

                        companyList = new string[dt.Rows.Count];

                        for (int j = 0; j < dt.Rows.Count; j++)
                        {
                            companyList[j] = dt.Rows[j][0].ToString();
                        }
                    }

                }

                companyList = companyList.Where(x => !string.IsNullOrEmpty(x)).ToArray();
                        ...
                        if (charges_company_new.Data is null)
                        {
                            foreach (Company item in result.Data.Items.Where(t => t as Company != null))
                            {
                                try
                                {
                                    ...
                                    //query = "INSERT INTO " + tbName + " (`Sr_No`, `Company_Name`, `Company_Name_Link`, `Company_Number`,`Company_Number_Link`, `Registered_Address`, `Charges`, `Date_Creation`, `Status`, `Persons_Entitled`, `Brief_Description`, `Charge_Code`, `Company_Link`) " +
                                    //     " VALUES (@srno, @cname, @cnamelink, @cnumb, @cnumblink, @radd, @charges, @dtCreate, @status, @pent, @bdesc, @ccode, @clink)";
                            ...
                            foreach (Charge charge_Company in charges_company_new.Data.Items.Where(t => t as Charge != null))
                            {
                                ...
                                foreach (Company item in result.Data.Items.Where(t => t as Company != null))
                                {
                                    try
                                    {
                                        ...
                                        //cmd.Parameters.AddWithValue("@srno", srNOChargesNew.ToString());
                                        //cmd.Parameters.AddWithValue("@cname", item.Title is null ? "NA" : item.Title.ToString());
                                        //cmd.Parameters.AddWithValue("@cnamelink", "https://app.creditsafe.com/search?countries=GB&limit=15&name=" + (item.Title is null ? "NA" : item.Title.Trim().ToString()) + "&page=1".ToString());
                                        //cmd.Parameters.AddWithValue("@cnumb", item.CompanyNumber is null ? "NA" : item.CompanyNumber.ToString());
                                        ...
                                        //cmd.Parameters.AddWithValue("@dtCreate", DateTime.Parse(charge_Company.CreatedOn.Value.ToShortDateString()).ToString("yyyy -MM-dd").ToString());
                                        //cmd.Parameters.AddWithValue("@status", charge_Company.Status.ToString());
                                        //cmd.Parameters.AddWithValue("@pent", charge_Company.PersonsEntitled[0].Name is null ? "NA" : charge_Company.PersonsEntitled[0].Name.ToString());
                                        //cmd.Parameters.AddWithValue("@bdesc", charge_Company.Particular.Description is null ? "NA" : charge_Company.Particular.Description.ToString());
        ...
        protected void btnPDF_Click(object sender, EventArgs e)
        {
            ...
            //string tbName = Session["username"] + "_companies";
            ////Session["username"] + "_sic_" + sicList.SelectedValue.ToString().Trim();
